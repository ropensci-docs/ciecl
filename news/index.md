...
[ropensci](https://github.com/ropensci/ciecl). El paquete se instala
además desde el [R-universe de
rOpenSci](https://ropensci.r-universe.dev) y su documentación se publica
en <https://docs.ropensci.org/ciecl>.

Cambios al usuario consolidados de la revisión:

- **Validación de entradas inválidas**: las funciones públicas abortan
  con errores tipados de clase `ciecl_invalid_input` (mensajes claros en
  lugar de errores base de R):
  [`cie_search()`](https://docs.ropensci.org/ciecl/reference/cie_search.md)
  valida `threshold`/`max_results`;
  [`cie_lookup()`](https://docs.ropensci.org/ciecl/reference/cie_lookup.md)
  exige `extract` escalar;
  [`cie_expand()`](https://docs.ropensci.org/ciecl/reference/cie_expand.md)
  valida `code` (con `NA` abortando en lugar de retornar `character(0)`
  silenciosamente);
  [`cie_table()`](https://docs.ropensci.org/ciecl/reference/cie_table.md)
  exige un único código y rechaza `NA`;
  ...
  valida y coacciona `default`.
- **Orden determinista en
  [`cie_lookup()`](https://docs.ropensci.org/ciecl/reference/cie_lookup.md)**:
  el modo vector exacto (`IN (...)`) ahora incluye `ORDER BY codigo`,
  igual que los demás caminos de la función.
- **Rangos inclusivos en
  [`cie_lookup()`](https://docs.ropensci.org/ciecl/reference/cie_lookup.md)**:
  un rango como `"E10-E14"` ya no excluye silenciosamente las
  subcategorías del límite superior.
- **Robustez de
  [`cie10_sql()`](https://docs.ropensci.org/ciecl/reference/cie10_sql.md)**:
  una consulta `SELECT` legítima que comienza con un comentario ya no
  aborta como “no SELECT”.
- **[`cie_search()`](https://docs.ropensci.org/ciecl/reference/cie_search.md)
  con texto de solo símbolos** (p. ej. `"!!"`): ya no deriva en un error
  interno.
...
class `ciecl_invalid_input` across public functions;
[`cie_lookup()`](https://docs.ropensci.org/ciecl/reference/cie_lookup.md)
is deterministic (`ORDER BY codigo`) in exact vector mode and its ranges
now include the upper limit’s subcategories;
[`cie10_sql()`](https://docs.ropensci.org/ciecl/reference/cie10_sql.md)
no longer rejects a legitimate `SELECT` starting with a comment; and
[`cie_search()`](https://docs.ropensci.org/ciecl/reference/cie_search.md)
handles symbol-only text.*

El ciclo de desarrollo previo (0.9.8 en desarrollo, 2026-04-25 →
2026-09-16) se publica íntegramente como 1.0.0; su detalle se conserva
en las secciones fechadas siguientes.
...
### Respuesta a comentarios rOpenSci [\#765](https://github.com/ropensci/ciecl/issues/765) (2026-09-08)
...
revisión rOpenSci [\#765](https://github.com/ropensci/ciecl/issues/765).
Sin cambios en `R/` ni en la API pública.
...
[\#765](https://github.com/ropensci/ciecl/issues/765). Sin cambios en la
API pública.
...
[\#765](https://github.com/ropensci/ciecl/issues/765), a partir de los
...
- **Sitio pkgdown**: footer de `_pkgdown.yml` corregido según el PR
  [\#15](https://github.com/ropensci/ciecl/issues/15) de la revisora
...
[\#765](https://github.com/ropensci/ciecl/issues/765). Sin cambios en la
API pública: no se agregan ni eliminan funciones exportadas, no cambian
firmas ni se rompe compatibilidad.
...
- **Limpieza técnica menor**:
  ...
  - Badge rOpenSci Software Peer Review
    ([\#765](https://github.com/ropensci/ciecl/issues/765)) agregado a
    los README.
...
review [\#765](https://github.com/ropensci/ciecl/issues/765). No changes
to `R/` or the public API.

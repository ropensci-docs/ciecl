...
skip this vignette: `pak::pak("ropensci/ciecl")` is all you need to get
started.
...
|----|----|----|
| Charlson/Elixhauser comorbidity indices with [`cie_comorbid()`](https://docs.ropensci.org/ciecl/reference/cie_comorbid.md) | `comorbidity` | `install.packages("comorbidity")` |
| Formatted HTML tables with [`cie_table()`](https://docs.ropensci.org/ciecl/reference/cie_table.md) | `gt` | `install.packages("gt")` |
...
> (Option B) or via `keyring` (Option A). The `api_key` argument of
> [`cie11_search()`](https://docs.ropensci.org/ciecl/reference/cie11_search.md)
...
[`usethis::edit_r_environ()`](https://usethis.r-lib.org/reference/edit.html)
if you have `usethis` installed) and add:
